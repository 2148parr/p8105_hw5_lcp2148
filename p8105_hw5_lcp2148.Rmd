...
path = "./data/problem_2/"
data_in = list.files("./data/problem_2/")  %>% 
  mutate(subject = map(path, ~ read_csv(file.path(path, .)))


files = dir(data_in, pattern = "*clean.csv")

data_1 = tibble(filename = files) %>%
  mutate(file_contents = map(filename, 
                             ~ read_csv(file.path(data_path, .)))) %>% 
           unnest()


read_cvs("./data/problem_2/con_01.csv") 
